...
import { MdShoppingCart, MdClose, MdEdit, MdCheck, MdWarning, MdSave } from 'react-icons/md'
...
export interface CartData {
  ...
  faixaAtual?: { minimo: number; maximo: number }
}

export interface CartEditResult extends CartData {
  invalidated?: boolean
  resetToPhase?: string
}
...
interface Props {
  data: CartData
  onEdit: (editedData: CartEditResult) => void
...
function stepCount(d: CartData) {
  let c = 0
  if (d.modalidade) c++
  if (d.terreno) c++
  if (d.quartos) c++
  if (d.planta) c++
  if (d.opcionais?.some(o => o.selecionado)) c++
  if (d.personalizacoes && d.personalizacoes.length > 0) c++
  return c
}
...
function faixaLabel(d: CartData): string {
  if (!d.faixaAtual) return 'R$ 0'
  return `${formatCurrency(d.faixaAtual.minimo)} — ${formatCurrency(d.faixaAtual.maximo)}`
}

export default function CarrinhoFlutuante({ data, onEdit, plantas, visible }: Props) {
  ...
  function checkIncompatibility(result: CartEditResult): { incompatible: boolean; msg: string; resetPhase: string } {
    ...
    if (areaFail || quartosFail) {
      ...
      return {
        incompatible: true,
        msg: `A planta "${p.nome}" será removida porque ${reasons.join(' e ')}. Os opcionais e personalizações também serão redefinidos. Deseja continuar?`,
        resetPhase: 'PLANTA',
      }
    }
  ...
  if (!expanded) {
    return (
      <button
        ...
        className="fixed top-4 right-4 z-40 btn btn-neutral shadow-xl rounded-full gap-2 transition-all duration-300 hover:scale-105"
      >
        <div className="relative">
          <MdShoppingCart size={20} />
          {count > 0 && <span className="badge badge-xs badge-accent absolute -top-2 -right-3">{count}</span>}
        </div>
        <span className="text-xs font-semibold">
          {data.faixaAtual ? faixaLabel(data) : 'R$ 0'}
        </span>
      </button>
    )
  }
  ...
  return (
    <>
      <div className="fixed top-4 right-4 z-40 card bg-base-100 shadow-2xl border border-base-300 w-96 max-h-[85vh] overflow-y-auto transition-all duration-300">
        <div className="card-body p-4 gap-3">
          <div className="flex items-center justify-between">
            <h3 className="font-bold text-sm tracking-wide uppercase text-base-content">Resumo do Projeto</h3>
            <button onClick={() => { setExpanded(false); setEditingKey(null) }} className="btn btn-ghost btn-xs btn-circle"><MdClose size={18} /></button>
          </div>
          ...
          <div className="flex flex-col gap-2">
            ...
            {count === 0 && <p className="text-xs text-base-content/50 text-center py-4">Nenhuma escolha definida ainda.</p>}
          </div>
          ...
          <div className="text-center py-1">
            ...
            ) : <p className="text-base-content/50 text-sm mt-1">R$ 0</p>}
          </div>
        </div>
      </div>
          ...
          <div className="card bg-base-100 shadow-2xl w-full max-w-md max-h-[80vh] overflow-y-auto" onClick={e => e.stopPropagation()}>
            <div className="card-body p-5 gap-4">
              <div className="flex items-center justify-between">
                ...
                <button onClick={() => setEditingKey(null)} className="btn btn-ghost btn-sm btn-circle"><MdClose size={18} /></button>
              </div>
